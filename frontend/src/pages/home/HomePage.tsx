...
const HomePage: React.FC<Props> = (props: Props) => {
  ...
  const onFileUpload = (acceptedFiles: File[]) => {
    ...
    setUploading(true);
    apiClient.uploadExport(zipFile).then((data) => {
      setUploading(false);
    }).catch((error) => {
      setUploading(false);
  ...
  return (
    <div className="mt-32 max-w-6xl flex flex-col justify-center items-center mx-auto my-auto">
      <main>
        <section>
          <h1 className="font-bold text-white text-4xl text-center mb-2">Apple Watch Workout Analysis Tool</h1>
          <p className="text-center text-lg">
          Upload your Apple Health export file to analyze all recorded workouts!
          </p>
        </section>
